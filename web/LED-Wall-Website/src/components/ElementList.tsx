...
// Combine Actions and Types from your configSlice
import {
  setSelectedElement,
  updateElement,
  addElement,
...
// This single line handles all the types you need for duplication/adding
import type { 
  Elem, 
  TextElem, 
  ImageElem, 
...
import ContextMenu, { type Option } from "./ContextMenu.tsx";
...
function ElementList(props: Props) {
  ...
  async function handleDrop(e: React.DragEvent<HTMLLIElement>, targetId: number) {
    ...
    const finalElements = reordered.map((el, index) => ({ ...el, id: index + 1 }));
  ...
  async function duplicateElement(id: number) {
    ...
    const newElement = { ...elementToCopy, id: 1, name: newName, visible: true };
  ...
  function handleRightClick(e: React.MouseEvent<HTMLLIElement>, id: number) {
    // We explicitly set the ID and pass it to the function
