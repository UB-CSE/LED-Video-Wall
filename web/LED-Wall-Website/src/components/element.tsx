...
import { clearElement, reorderElements } from "../state/config/configSlice.ts";
...
function Element(props: ElementProps) {
  ...
  function handleContextMenu(e: React.MouseEvent) {
    e.preventDefault();
    setContextLocation([e.clientX - 240, e.clientY - 60]);
  ...
  function deleteElement() {
    ...
    dispatch(clearElement(props.id));
    ...
    if (props.type === "image") {
      return (
        <div onContextMenu={handleContextMenu}>
          ...
          />
          {contextIsClicked && <ContextMenu options={[{ name: "delete", function: deleteElement }]} location={contextLocation} />}
        </div>
      );
