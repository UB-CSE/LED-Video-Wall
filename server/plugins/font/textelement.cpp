...
Element renderTextToElement(const std::string &text,
  ...
  // Baseline position with some padding
  int x = 10;
  int y = estimatedHeight / 2; // Start with a reasonable baseline
...
int test_render() {
  VirtualCanvas vCanvas(cv::Size(64 * 2, 32 * 2));

  Element textElement =
      renderTextToElement("Hello Matrix!", "./ttf/Roboto-Regular.ttf", 18,
                          cv::Scalar(255, 0, 0), 1, cv::Point(5, 5));

  vCanvas.addElementToCanvas(textElement);

  cv::imwrite("outfile.png", vCanvas.getPixelMatrix());
