...
protected:
  ...
  Element(int id, cv::Point loc, int frameRate, double rotationDegrees = 0.0,
          double scaleFactor = 0.0)
      : id(id), location(loc), frameRate(frameRate),
        angleDegrees(rotationDegrees), scaleFactor(scaleFactor) {}
};
...
public:
  ImageElement(const std::string &filepath, int id, cv::Point loc, double scale,
...
private:
  ...
  size_t current; // This is the internal counter for carousel objects to
                  // remember which frame they are on

public:
  CarouselElement(const std::vector<std::string> &filepaths, int id,
                  cv::Point loc, int frameRate, double rotationDegrees = 0.0);
...
public:
  VideoElement(const std::string &filepath, int id, cv::Point loc,
               int frameRate, double rotationDegrees = 0.0);
...
private:
  ...
  const cv::Mat noFrameMat =
      cv::Mat(100, 100, CV_8UC3, cv::Scalar(0, 255, 0)); // green

public:
  RTMPStreamElement(RTMPServer &rtmpServer, const std::string &streamName,
...
private:
  ...
  const cv::Mat noFrameMat =
      cv::Mat(size.width, size.height, CV_8UC3, cv::Scalar(255, 0, 0)); // red

public:
  WebBrowserElement(const std::string &url, int id, cv::Point loc,
                    int frameRate, cv::Size size,
...
public:
  ...
  TextElement(const cv::Mat &imgBGR, int id, cv::Point loc,
              const std::string &text, const std::string &font, int size,
              cv::Scalar col, double rotationDegrees = 0.0);
};

// Originally, elementCount and PixelMatrix and the rest were in private, but
// for my MPI implementation, i needed to access them directly to init vCanvas
// with a default constructer
class VirtualCanvas {
...
public:
  ...
  // Keeping getPixelMatrix's matrix return instead of what I'm doing for
  // nextFrame in order to cut down on code merging time
...
class Element : public AbstractCanvas {
    ...
    public:
        ...
        Element(const std::string& path, int elementId, cv::Point loc =
cv::Point(0, 0), std::tuple<int, int> frameRateData = std::make_tuple(-1,0));
        Element(const cv::Mat, int elementId, cv::Point loc = cv::Point(0, 0),
std::tuple<int, int> frameRateData = std::make_tuple(-1,0));
