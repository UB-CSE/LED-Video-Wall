...
Event::Event(ns_ts timestamp, std::function<bool(Controller *)> action)
    : timestamp(timestamp), period(std::nullopt), action(action) {}

Event::Event(ns_ts timestamp, ns_dur period,
...
EventQueue::EventQueue() : queue(eventCompare) {}

void EventQueue::addEvent(Event evnt) { this->queue.insert(evnt); }

std::optional<Event> EventQueue::tryPopEvent(ns_ts cutoff_time) {
  auto next_event_it = this->queue.begin();
  if (next_event_it != this->queue.end() &&
      next_event_it->timestamp < cutoff_time) {
