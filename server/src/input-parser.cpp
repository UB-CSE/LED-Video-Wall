...
void parseInput(VirtualCanvas &vCanvas, std::string &inputFile,
  ...
  try {
    ...
    */
    ...
    for (YAML::const_iterator it = elements.begin(); it != elements.end();
         ++it) {
      ...
      */
      ...
      if (type == "image") {
        if (!value["filepath"] || !value["location"]) {
          std::cerr << "Missing filepath or location for element: " << key
                    << std::endl;
        }
        ...
        if (locVec.size() != 2 || locVec[0] < 0 || locVec[1] < 0) {
          std::cerr << "Location for element " << key << " malformed."
                    << std::endl;
        }
        ...
        */
        ...
        Element *elem =
            new ImageElement(filepath, id, loc, scale, rotationDegrees);
      ...
      */
        ...
        if (!value["filepaths"] || !value["location"] || !value["framerate"]) {
          std::cerr << "Missing filepaths, framerate, or location for element: "
                    << key << std::endl;
        }

        int frameRate = value["framerate"].as<int>();
        std::vector<std::string> filepaths =
            value["filepaths"].as<std::vector<std::string>>();
        std::vector<int> locVec = value["location"].as<std::vector<int>>();
        cv::Point loc(locVec[0], locVec[1]);
        ...
        Element *elem =
            new CarouselElement(filepaths, id, loc, frameRate, rotationDegrees);
        vCanvas.addElementToCanvas(elem);
      }
      ...
      */
        ...
        Element *elem =
            new VideoElement(filepath, id, loc, frameRate, rotationDegrees);
      ...
      */
        ...
        if (!value["camera-number"] || !value["location"] ||
            !value["framerate"]) {
          std::cerr << "Missing filepath or location for element: " << key
                    << std::endl;
        }
        ...
        Element *elem =
            new VideoElement(webcamNum, id, loc, frameRate, rotationDegrees);
      ...
      */
        ...
        if (!value["content"] || !value["font_path"] || !value["location"] ||
            !value["color"] || !value["size"]) {
          std::cerr << "Missing required values for element: " << key
                    << std::endl;
          continue;
        }
        ...
        // convert location to cv::Point (same validation style as other
        // branches)
        std::vector<int> locVec = value["location"].as<std::vector<int>>();
        if (locVec.size() != 2 || locVec[0] < 0 || locVec[1] < 0) {
          std::cerr << "Location for element " << key << " malformed."
        ...
        Element *elem =
            renderTextToElement(content, filepath, fontSize, fontColor, id,
                                posPoint, rotationDegrees);
        if (!elem) {
          std::cerr << "Error parsing config: text failed to render (check TTF "
                       "path/permissions): "
                    << filepath << std::endl;
          continue;
        }
        ...
        if (!value["stream-name"] || !value["location"] ||
            !value["framerate"]) {
          std::cerr
              << "Missing stream-name, location, or framerate for RTMP element:"
              << key << std::endl;
          continue;
        }
        ...
        if (locVec.size() != 2 || locVec[0] < 0 || locVec[1] < 0) {
          std::cerr << "Location for the element" << key << " malformed"
                    << std::endl;
          continue;
        }
        ...
        if (value["size"]) {
          ...
          if (sizeVec.size() != 2 || sizeVec[0] <= 0 || sizeVec[1] <= 0) {
            std::cerr << "Size for the element" << key << " malformed"
                      << std::endl;
            continue;
          }
        ...
        Element *elem = new RTMPStreamElement(rtmpServer, streamName, id, loc,
                                              frameRate, size, rotationDegrees);
        ...
        if (!value["url"] || !value["location"] || !value["framerate"] ||
            !value["size"]) {
          std::cerr << "Missing url, location, framerate, or size for browser "
                       "element:"
                    << key << std::endl;
          continue;
        }
        ...
        if (value["view-size"]) {
          std::vector<int> viewSizeVec =
              value["view-size"].as<std::vector<int>>();

          if (viewSizeVec.size() != 2 || viewSizeVec[0] <= 0 ||
              viewSizeVec[1] <= 0) {
            std::cerr << "View Size for the element" << key << " malformed"
                      << std::endl;
            continue;
          }
        ...
        WebBrowserElement *elem = new WebBrowserElement(
            url, id, loc, frameRate, size, viewSize, rotationDegrees);
          ...
          for (const auto &cookie : value["cookies"]) {
            ...
            if (cookie["sameSite"]) {
              ...
              if (sameSiteStr == "None") {
                ...
                std::cerr << "Invalid sameSite value for cookie: "
                          << sameSiteStr << std::endl;
              }
            }

            elem->setCookie(name, value, domain, path, secure, httpOnly,
                            sameSite);
          }
        }
