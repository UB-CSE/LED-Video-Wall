...
parse_matrix_specs(YAML::Node ynode_matrix_specs) {
  ...
  for (YAML::const_iterator it = ynode_matrix_specs.begin();
       it != ynode_matrix_specs.end(); ++it) {
    ...
    float power_limit =
        yaml_key_present_and_unique(spec_node, "power_limit_amps").as<float>();
    YAML::Node width_height_node =
        yaml_key_present_and_unique(spec_node, "width-height");
...
parse_matrices(YAML::Node ynode_matrices,
  ...
  for (YAML::const_iterator it = ynode_matrices.begin();
       it != ynode_matrices.end(); ++it) {
    std::string id = it->first.as<std::string>();
    ...
    std::string spec_id =
        yaml_key_present_and_unique(matrix_node, "spec").as<std::string>();
    ...
    std::optional<rotation> rot_opt =
        parse_rotation(rot_node.as<std::string>());
    ...
    CanvasPos pos = CanvasPos(x, y, width, height, rot);
...
bool overlaping_range(uint32_t i, uint32_t iwidth, uint32_t j,
                      uint32_t jwidth) {
  ...
  for (LEDMatrix *i : mat_vec) {
    for (LEDMatrix *j : mat_vec) {
      ...
      if (overlaping_range(i->pos.x, i->pos.width, j->pos.x, j->pos.width)) {
        if (overlaping_range(i->pos.y, i->pos.height, j->pos.y,
                             j->pos.height)) {
          std::stringstream ss;
...
std::vector<Client *>
parse_clients(YAML::Node ynode_clients,
              std::map<std::string, LEDMatrix *> matrices) {
  std::vector<Client *> clients;

  for (YAML::const_iterator it = ynode_clients.begin();
       it != ynode_clients.end(); ++it) {
    ...
    if (!mac_addr_opt.has_value()) {
      throw YAML::RepresentationException(it->first.Mark(),
                                          "Invalid MAC Address!");
    }
    uint64_t mac_addr = mac_addr_opt.value();

    bool unique =
        std::find_if(clients.begin(), clients.end(), [mac_addr](Client *c) {
          return c->mac_addr == mac_addr;
        }) == clients.end();
    if (!unique) {
      std::stringstream ss;
      throw std::logic_error("Multiple clients with MAC address '" +
                             mac_addr_str + "'!");
    }

    YAML::Node connections_node =
        yaml_key_present_and_unique(it->second, "matrix-connections");
    ...
    for (size_t i = 0; i < connections_node.size(); ++i) {
      ...
      conn.pin = pin;
      YAML::Node matrices_node =
          yaml_key_present_and_unique(connection_node, "matrices");
...
ServerConfig parse_config_throws(std::string file) {
  ...
  YAML::Node ynode_matrix_specs =
      yaml_key_present_and_unique(config, "matrix-specs");
  YAML::Node ynode_ignore_bounds_checks = config["ignore-bounds-checks"];
  YAML::Node ynode_ns_per_frame =
      yaml_key_present_and_unique(config, "ns-per-frame");

  // Parse ns per frame
  int64_t ns_per_frame = ynode_ns_per_frame.as<int64_t>();
  if (ns_per_frame <= 0) {
    throw YAML::RepresentationException(
        ynode_ns_per_frame.Mark(),
  ...
  // Do bounds checks
  if (!(ynode_ignore_bounds_checks &&
        (ynode_ignore_bounds_checks.as<std::string>() == "true"))) {
