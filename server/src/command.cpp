...
/*
inputAvailable is boilerplate code from my past projects. This is intended to
allow stdin to be read afterwards without blocking from waiting for input.
...
int processCommand(VirtualCanvas &vCanvas, const std::string &line,
  ...
  if (cmd == "add") {
    ...
    if (!(iss >> type >> id >> filepath >> x >> y >> scale) || x < 0 || y < 0) {
      std::cerr << "Invalid add. Usage:\n add <type> <ElementID> <filepath> "
                   "<x> <y>\n";
    } else {
      Element *newelement =
          new ImageElement(filepath, id, cv::Point(x, y), -1, scale);
  ...
  if (cmd == "rotate") {
    ...
    if (!(iss >> id >> rotationDegrees)) {
      std::cerr << "Invalid rotate. Usage:\n  rotate <ElementID> <degrees>\n";
    } else {
      vCanvas.rotateElement(id, rotationDegrees);
    }
    return 0;
  }
  if (cmd == "set_rotation") {
    ...
    double rotationDegrees;
    if (!(iss >> id >> rotationDegrees)) {
      std::cerr << "Invalid set_rotation. Usage:\n  set_rotation <ElementID> "
                   "<degrees>\n";
    } else {
      vCanvas.setElementRotation(id, rotationDegrees);
    }
    return 0;
  }
  ...
  if (cmd == "set_font") {
    ...
    TextElement *newElem = dynamic_cast<TextElement *>(renderTextToElement(
        currentText, newFontPath, fontSize, color, id, loc));
    if (!newElem) {
  ...
  if (cmd == "scale") {
    ...
    if (!(iss >> id >> factor) || factor <= 0.0) {
      std::cerr << "Invalid scale. Usage:\n  scale <ElementID> <factor> (e.g., "
                   "0.5)\n";
    } else {
      vCanvas.setElementScale(id, factor);
    }
